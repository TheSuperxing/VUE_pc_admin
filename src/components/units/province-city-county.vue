...
<script>
  ...
  export default {
    ...
    data(){
      return{
        province:{//获取省市区数据
          provinceData:provinceArr,//省份数据，初始化
          cityData:provinceArr[0].children,//市数据，初始化
          countyData:provinceArr[0].children[0].children,//区的数据，初始化
        },
        reveal:{
...
//      },
    ...
    methods:{
      ...
      selectCity(){//添加模式下选择市
        ...
        for(var i=0;i<this.province.cityData.length;i++){
          if(this.province.cityData[i].name==this.accptProvince.city){
            Vue.set(this.province,"countyData",this.province.cityData[i].children);

            if(btsc){//初始化显示数据的时候如果btsc时从0初始化，否则从1初始化
              Vue.set(this.accptProvince,"county",provinceArr[this.reveal.provices.proviceIndex].children[i].children[0].name);//区的初始化，处理城市改变，区空白
            }else {
              Vue.set(this.accptProvince,"county",provinceArr[this.reveal.provices.proviceIndex].children[i].children[1].name);//区的初始化，处理城市改变，区空白
            }
          }
        }
        if(!btsc){//直辖市从0开始赋值
          this.province.countyData.splice(0,1)//当不是北上天重时，移除第一项（市辖区）
        }
